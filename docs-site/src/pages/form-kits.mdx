...
A control receives a field value from React Hook Form (RHF). It renders the interactive
...
Connect `input` and `meta.invalid` to the interactive element. Read [Connect a
custom control](/accessibility#connect-a-custom-control) for the contract.

The metadata has these meanings:
...
Read [Option values](/definitions#option-values) for definition examples.
...
| `radio` | `string \| undefined` | No props; items come from field `options`. |
| `checkbox` | `boolean` | No props. |
| `file` | `File \| undefined` | `accept` |

The `select` and `radio` controls require field `options`. The `radio`
control represents `undefined` as a group with no checked item. Add `props.emptyOption` when the
...
After validation succeeds, `onSubmit` receives four values. See [`onSubmit`
...
Read [Native controls](/api#native-controls) and [Default
slots](/api#default-slots) in the API reference for factory imports and
localization types. Read [Localization](/localization) to translate the labels.

## Next steps

- [Validate the input and submit the output](/validation)
- [Style the generated form](/styling)
