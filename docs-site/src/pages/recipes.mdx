...
| Apply a raw React Hook Form (RHF) update to multiple fields | `form.api.setValues(values)` |
...
| Add a design-system control | `defineControl` and the [accessibility contract](/accessibility#connect-a-custom-control) |

Read [Ownership](/how-it-works#ownership) for the parts that React Hook Form
and the application own.
...
server's canonical schema input when the server returns different
values.
...
For one definition-owned rule, use `beforeUpdate` to adjust or cancel the
proposal and `afterUpdate` to observe its final committed transaction. Configure
both in `defineForm`. Use middleware when independent policies must compose in
an explicit order.
...
Generated controls and `form.update` make one RHF value
[publication](/glossary#publication). Generated
...
contains an invalid field. Hidden fields [keep their
values](/how-it-works#hidden-fields-keep-their-values) when the next step
renders.
...
A transport failure is not a schema issue. See [Server issues are not schema
...
Standard Schema can transform data. In that case, schema input and submitted
...
`onSubmit` receives the parsed `value` and the editable `input`. See
[`onSubmit` receives four values](/how-it-works#onsubmit-receives-four-values).

Send `value` when the server expects parsed output. Pass schema input to
`form.api.reset`. If the server returns only output, convert it to schema
input at the application boundary.

Keep schema parsing deterministic and free of side effects. See [The schema
...
A custom control or slot must attach the supplied IDs, refs, and state to its
elements. Read [Accessibility](/accessibility) for the complete contract.
...
- [Make a custom control accessible](/accessibility)
