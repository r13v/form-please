---
title: Definitions
description: Describe fields, sections, arrays, reusable fragments, derived UI, and selectable options in one typed tree.
showAskAi: false
searchPriority: 2
---
...
A definition connects one Standard Schema to the UI and managed-update policy
that govern its input. Call `kit.defineForm(schema, source, options?)` outside
the component that uses the form. The schema-bound helpers keep paths,
controls, options, and resolver values connected. They also set the internal
node type, so you never write it.
...
- Control props, selectable options, and slot options use their
  registered types.
...
The optional third argument configures `beforeUpdate`, ordered `middleware`,
and `afterUpdate`. These callbacks and a frozen middleware snapshot become
readonly properties of the definition. See [Value middleware](/middleware) for
their managed-update boundary and ordering.
...
The `required` property describes the UI state only. See [`required` is
...
and keeps the concrete schema available as `fragment.schema`. Compose that
schema into a form schema, then place the fragment at each compatible object path:
...
An array node connects a non-nullable object-array path to the definition for
one item. Paths in `children` are relative to that item.
...
Generated field paths use React Hook Form (RHF) dot syntax. The title above can resolve to
...
Display, interaction, control props, and layout properties accept
either a static value or a synchronous resolver.
...
Use an application resource or a custom control when you need loading or error
UI, caching, retries, remote search, or pagination.
...
ui.section("billing", {
	...
	children: [
		ui.field("billingAddress.street", { control: "text", label: "Street" }),
	],
})
```
...
Field properties. `ui.field(path, options)` takes the path as its first
argument:
...
Section properties. `ui.section(id, options)` takes the ID as its first
argument:
...
Array properties. `ui.array(path, options)` takes the path as its first
argument:

| Property | Required | Purpose |
| --- | --- | --- |
| `path` | Yes | Selects a non-nullable object-array path in the current schema scope. |
| `itemDefault` | Yes | Supplies one complete item for the add action. |
| `children` | Yes | Receives a builder bound to one array item and returns its nodes. |
| `label` | No | Supplies string or React label content. |
| `description` | No | Supplies string or React help content. |

Render properties. `ui.render(id, options)` takes the ID as its first
argument:
...
See [Form kits](/form-kits) for control props, selectable options, slot options, and
...
```tsx
ui.field("profile.email", { control: "text" })
ui.field("speakers.0.name", { control: "text" })
```
...
- [Use my design system](/form-kits)
