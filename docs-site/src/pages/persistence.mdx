...
Create one feature, add that exact reference to `middleware`, and get its handle
from the resulting form. Start `restore()` in an effect. It loads at most once
...
This is safe for client modules that can also render on a server.
...
Call `start()` instead of `restore()` when the application does not want an
initial load. A normal start does not write the unchanged initial values. A
start after a conflict or restore failure immediately saves the current input.
...
The handle provides these operations:
