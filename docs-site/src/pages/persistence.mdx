...
needs a durable editable draft. React Hook Form (RHF) still owns the live values and
form state, as [Ownership](/how-it-works#ownership) describes. Persistence
stores only the complete schema input.

## Configure and restore

Create one feature, add that exact reference to the definition's `middleware`,
and pass the form and feature to `usePersistence`. The hook subscribes to
persistence state and starts `restore()` after mount. It loads at most once
after a successful or empty result.
...
`version`, and the encoded payload. Increase `version` when the schema input
...
- [Save a draft in the query string](/examples/persistence)
