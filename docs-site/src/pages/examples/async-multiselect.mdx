...
The options contract separates request settings from display text:
...
"Form, Please" resolves field options synchronously. Pass the asynchronous
function as `queryFn`. Do not return a promise from the `options` resolver.
...
The control requires these request options:
...
These options control request and label behavior:
...
These options control visible text and tag count:
