...
| `form-please/history` | `createHistoryMiddleware` | Add optional managed value history and journal transfer. |
| `form-please/native-controls` | `createNativeControls` | Create the native HTML control registry. |
| `form-please/persistence` | `createPersistenceMiddleware`, `createLocalStorageAdapter`, `createDateCodec` | Restore and autosave editable drafts through application storage. |
...
persistence entries have no React component or hook dependency.
...
`kit.defineFragment(schema, { ui })` returns a frozen fragment with the exact
input schema at `fragment.schema` and placement methods at `fragment.fields`.
Fragment-authored resolvers receive the local fragment input. See [Reusable
fragments](/definitions#reusable-fragments) for composition, context, and array
examples.
...
Select and radio `choices`, plus autocomplete `options`, narrow to the schema
...
Each supported property accepts a static value or a synchronous resolver. An
...
`createHistoryMiddleware(options?)` is exported only from
`form-please/history`. Configure the returned feature in a form's middleware
list, then call `feature.handle(form)` for the exact form-specific handle.
...
`createPersistenceMiddleware(options)` is exported only from
`form-please/persistence`. Configure its feature in the fixed middleware list,
then call `feature.handle(form)`.
