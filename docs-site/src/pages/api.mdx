...
behavior. Form, Please adds typed UI definitions to React Hook Form (RHF). Each form
...
| `form-please/testing` | `createDefinitionTester` | Inspect resolved definitions and exercise production managed updates without rendering React. |
...
The testing entry is a server-compatible headless module.
...
Import `createDefinitionTester(definition, options)` from
`form-please/testing`. It creates a stateful headless harness around the
production definition resolver, managed value coordinator, and field-options
executor.

| Member | Result |
| --- | --- |
| `current`, `values` | Current readonly resolution and complete schema input. |
| `field(path)`, `array(path)`, `node(id)` | Strict selectors with candidate-rich errors. |
| `setValue(path, value)`, `update(recipe)` | Managed control or imperative proposals through hooks and middleware. |
| `append(path)`, `remove(path, index)`, `move(path, from, to)` | Managed generated-array proposals. |
| `setContext`, `setDisabled`, `setReadOnly`, `rerender` | Re-resolve non-value form inputs without middleware. |
| `resolveOptions(path)` | Resolve static or asynchronous field options and expose failures. |

Each operation returns a transition with `before`, `after`, and exact public
node `changes`. Managed transitions also report `committed`, `transaction`, and
the middleware `result`. Read [Test definitions](/testing) for complete examples
and boundaries.
...
| `radio` | `string \| undefined` | Field `options` |
...
Import `DefaultSlotsI18n` and its related types from
`form-please/default-slots`. Read
[Localization](/localization#translate-the-form-kit-labels) for the keys and
their callback data.
...
Read [Localization](/localization#translate-the-form-kit-labels) for the keys
of the `i18n` option.
...
`kit.defineForm(schema, source, options?)` validates, normalizes, and freezes a
...
they are not value resolvers.

A builder list accepts only nodes from the helpers of its own scope and
fragment placements. A root helper inside `array.children` causes a type
error. Use the `{ ui }` object form to write nodes as objects.

The optional third argument owns the complete managed-update policy:

| Option | Effect |
| --- | --- |
| `beforeUpdate` | Adjust or cancel one managed value proposal before middleware. |
| `middleware` | Ordered Redux-shaped value middleware. |
| `afterUpdate` | Observe the final committed transaction after middleware unwinds. |

The callbacks and a frozen copy of the middleware list become readonly
properties of the definition. Each form binding and definition tester creates
its own middleware instances from that fixed policy. Use the current
`transaction.context` for binding-specific data.

In addition to opaque fragment placements, the builder creates four ordinary
node kinds:

| Helper | Required options | Purpose |
| --- | --- | --- |
| `ui.field(path, options)` | `control` | Connect one schema input path to one registered control. |
| `ui.section(id, options)` | `children` | Group nodes without changing their path scope. |
| `ui.array(path, options)` | `itemDefault`, `children` | Repeat nodes for a non-nullable object-array path. |
| `ui.render(id, options)` | `component` | Insert form-local React content. |
...
and [`required` is UI-only](/how-it-works#required-is-ui-only).
...
| `onSubmit` | Receives parsed output, schema input, the current binding, and a submitter snapshot. |
...
`onSubmit` runs only after the Standard Schema parse succeeds. See [`onSubmit`
...
Configure `beforeUpdate` in the third argument to `defineForm` for one
definition-owned rule that adjusts or cancels a managed proposal. Its Immer
draft starts with the proposed `nextValues`. Mutate the draft and return
nothing to continue, or return `false` to cancel the complete transaction.
...
The callbacks are fixed with the definition. Read current binding-specific data
from `transaction.context`. Both must return synchronously and cannot start
another managed update. An error from
...
one proposal, or when work must run only after that proposal commits. Put the
ordered list in the third argument to `defineForm`.
...
only from `form-please/history`. Configure the returned feature in the
definition's middleware list, then pass it and the form to the hook. The hook
result combines the exact form-specific handle with its reactive `snapshot`.
...
exported only from `form-please/persistence`. Configure the feature in the
definition's fixed middleware list, then pass it and the form to the hook. The
hook starts restore after mount and combines the exact handle with its reactive
`snapshot`.
