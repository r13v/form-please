...
Use `kit.Form` and `kit.Fields` for manual composition. A manually composed
`kit.Form` has no summary. The application must render its own summary for
form-level and hidden-field issues.
